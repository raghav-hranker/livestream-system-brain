...
| **`pdfAsset`** | `{bucket:'documents', key, mimeType:'application/pdf', sizeBytes, sha256?}` on the `Pdf`; the `/access` routes sign it into a short-lived Bunny URL. PDF analogue of `hlsAsset`; `bucket` is a **logical zone alias**, never a physical bucket name. `key` is opaque — do not infer content hashes from migrated key filenames. | `repos/nodejs-server/docs/plans/prd-secure-pdf-delivery.md` + `docs/adr/0002-documents-zone-and-exact-file-tokens.md` | nodejs-server (owner/signer); admin-dashboard (uploads produce it, reads derived `state` only) |
| **`documents` zone** | The private PDF store: physical B2 bucket `hranker-private-assets`, Bunny signed reads only, uploads via B2's S3 endpoint. Distinct from the public R2 path that images/banners stay on. | `repos/nodejs-server/docs/adr/0002-documents-zone-and-exact-file-tokens.md` | nodejs-server (owns credentials + presigning); admin-dashboard (only ever sees signed URLs / presigned PUTs) |
| **PDF upload session** *(planned)* | Server-owned bulk-upload state machine (`pending → target_issued → completed / failed / expired`; `uploading`/`uploaded`/`finalizing` are browser-derived presentation states only): per-file short-lived single-object presigned B2 `PUT` targets, mandatory verified idempotent completion, partial success. The browser is never the source of truth. | `docs/adr/0005-bulk-pdf-uploads-use-presigned-put.md` + `docs/plans/prd-secure-pdf-upload.md` (until it lands in `repos/nodejs-server`) | admin-dashboard (drives), nodejs-server (owns) |
